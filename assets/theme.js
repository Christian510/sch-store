...
function emptyCart() {
    let cart_notes = document.getElementById("CartSpecialInstructions").value;
    fetch('cart/clear.js', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
...
// CLEARS CART NOTES FROM DB //
function emptyCartNotes() {
    fetch('cart/update.js', {
        ...
        .then(response => {
            console.log(`Successfully cleared notes: ${response}`);
        })
        .catch(error => {
            console.log(`There was an error clearing notes: ${error}`);
        });
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Display donation modal fixed and allow scrolling in background. //
    setTimeout(function() {
        let pageContainerClass = document.getElementById('PageContainer');
        const mediaQuery = window.matchMedia("(min-width: 750px)");
        if (donationOverlay && donationOverlay) {
            donationOverlay.classList.replace('hidden', 'visible');
            donationModal.classList.replace('hidden', 'visible');
        }
        if (mediaQuery) {
            pageContainerClass.classList.remove('page-container');
        }
    }, 2500);
});

// CHANGE MODAL TO DISPLAY ERROR MESSAGE //
// function errorModal() {
